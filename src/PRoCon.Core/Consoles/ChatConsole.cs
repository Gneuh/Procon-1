    ...
    public class ChatConsole : Loggable {
        ...
        private void m_prcClient_ProconAdminYelling(PRoConClient sender, string strAdminStack, string strMessage, int iMessageDuration, CPlayerSubset spsAudience) {
            ...
            if (spsAudience.Subset == CPlayerSubset.PlayerSubsetType.All) {
                ...
                formattedMessage = String.Format("^b^2{0}^0 -^2 {1}^0 >^2 {2}", strAdminName.ToUpper(), this.m_prcClient.GetLocalizedTeamName(spsAudience.TeamID, this.m_prcClient.CurrentServerInfo.Map, this.m_prcClient.CurrentServerInfo.GameMode).ToUpper(), strMessage.ToUpper());
            }
            else if (spsAudience.Subset == CPlayerSubset.PlayerSubsetType.Squad) {
                formattedMessage = String.Format("^b^2{0}^0 -^2 {1}^0 - ^2{2}^0 >^2 {3}", strAdminName.ToUpper(), this.m_prcClient.GetLocalizedTeamName(spsAudience.TeamID, this.m_prcClient.CurrentServerInfo.Map, this.m_prcClient.CurrentServerInfo.GameMode).ToUpper(), this.m_prcClient.Language.GetLocalized("global.Squad" + spsAudience.SquadID.ToString(), null), strMessage).ToUpper();
            }
        ...
        private void m_prcClient_ProconAdminSaying(PRoConClient sender, string strAdminStack, string strMessage, CPlayerSubset spsAudience) {
            ...
            if (spsAudience.Subset == CPlayerSubset.PlayerSubsetType.All) {
                ...
                formattedMessage = String.Format("^b^2{0}^0 -^2 {1}^0 >^2 {2}", strAdminName, this.m_prcClient.GetLocalizedTeamName(spsAudience.TeamID, this.m_prcClient.CurrentServerInfo.Map, this.m_prcClient.CurrentServerInfo.GameMode), strMessage);
            }
            else if (spsAudience.Subset == CPlayerSubset.PlayerSubsetType.Squad) {
                formattedMessage = String.Format("^b^2{0}^0 -^2 {1}^0 - ^2{2}^0 >^2 {3}", strAdminName, this.m_prcClient.GetLocalizedTeamName(spsAudience.TeamID, this.m_prcClient.CurrentServerInfo.Map, this.m_prcClient.CurrentServerInfo.GameMode), this.m_prcClient.Language.GetLocalized("global.Squad" + spsAudience.SquadID.ToString(), null), strMessage);
            }
            ...
            if (String.Compare(rawChat[0], "server", true) != 0) {
                if (rawChat.Count == 2) { // < R9 Support.
                    ...
                    if (int.TryParse(rawChat[3], out iTeamID) == true) {
                        formattedMessage = String.Format("^b^4{0}^0 - ^4{1}^0 >^4 {2}", rawChat[0], this.m_prcClient.GetLocalizedTeamName(iTeamID, this.m_prcClient.CurrentServerInfo.Map, this.m_prcClient.CurrentServerInfo.GameMode), rawChat[1]);
                    }
                }
                else if (rawChat.Count >= 5 && String.Compare(rawChat[2], "squad", true) == 0) {
                    if (int.TryParse(rawChat[3], out iTeamID) == true) {
                        if (String.Compare(rawChat[4], "0") != 0) {
                            formattedMessage = String.Format("^b^4{0}^0 - ^4{1}^0 - ^4{2}^0 >^4 {3}", rawChat[0], this.m_prcClient.GetLocalizedTeamName(iTeamID, this.m_prcClient.CurrentServerInfo.Map, this.m_prcClient.CurrentServerInfo.GameMode), this.m_prcClient.Language.GetLocalized("global.Squad" + rawChat[4], null), rawChat[1]);
                        }
                        else {
                            // TO DO: Localize and change uscPlayerListPanel.lsvPlayers.colSquad 
                            formattedMessage = String.Format("^b^4{0}^0 - ^4{1}^0 - ^4{2}^0 >^4 {3}", rawChat[0], this.m_prcClient.GetLocalizedTeamName(iTeamID, this.m_prcClient.CurrentServerInfo.Map, this.m_prcClient.CurrentServerInfo.GameMode), this.m_prcClient.Language.GetLocalized("uscPlayerListPanel.lsvPlayers.colSquad", null), rawChat[1]);
                        }
                    }
                }
        ...
        private void m_prcClient_PlayerKilled(PRoConClient sender, Kill kKillerVictimDetails) {
            if (this.LogKills == true) {
                ...
                bool bl_IsTK = false;
                if (kKillerVictimDetails.Killer.TeamID == kKillerVictimDetails.Victim.TeamID && kKillerVictimDetails.Killer.SoldierName != kKillerVictimDetails.Victim.SoldierName) { bl_IsTK = true; }
                ...
                if (kKillerVictimDetails.Headshot == false) {
                    if (kKillerVictimDetails.DamageType.Length > 0) {
                        if (bl_IsTK == true) {
                            this.Write(DateTime.UtcNow.ToUniversalTime().AddHours(m_prcClient.Game.UTCoffset).ToLocalTime(), String.Format("{0} [^3{1}^0] {2} [^8{3}^0]", strKillerName, this.m_prcClient.Language.GetLocalized(String.Format("global.Weapons.{0}", kKillerVictimDetails.DamageType.ToLower())), strVictimName, this.m_prcClient.Language.GetLocalized("uscChatPanel.chkDisplayOnKilledEvents.TeamKill")));
                        } else {
                    ...
                    if (kKillerVictimDetails.DamageType.Length > 0) {
                        if (bl_IsTK == true) {
                            this.Write(DateTime.UtcNow.ToUniversalTime().AddHours(m_prcClient.Game.UTCoffset).ToLocalTime(), String.Format("{0} [^3{1}^0] {2} [^2{3}^0] [^8{4}^0]", strKillerName, this.m_prcClient.Language.GetLocalized(String.Format("global.Weapons.{0}", kKillerVictimDetails.DamageType.ToLower())), strVictimName, this.m_prcClient.Language.GetLocalized("uscChatPanel.chkDisplayOnKilledEvents.HeadShot"), this.m_prcClient.Language.GetLocalized("uscChatPanel.chkDisplayOnKilledEvents.TeamKill")));
                        } else {
                        ...
                        if (bl_IsTK == true) {
                            this.Write(DateTime.UtcNow.ToUniversalTime().AddHours(m_prcClient.Game.UTCoffset).ToLocalTime(), String.Format("{0} [^3{1}^0] {2} [^2{3}^0] [^8{4}^0]", strKillerName, this.m_prcClient.Language.GetLocalized("uscChatPanel.chkDisplayOnKilledEvents.Killed"), strVictimName, this.m_prcClient.Language.GetLocalized("uscChatPanel.chkDisplayOnKilledEvents.HeadShot"), this.m_prcClient.Language.GetLocalized("uscChatPanel.chkDisplayOnKilledEvents.TeamKill")));
                        } else {
